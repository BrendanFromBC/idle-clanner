...
function IconDashboard() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-8 w-8">
      <path d="M12 2L4 7v6c0 5 3.5 9.3 8 10.5C16.5 22.3 20 18 20 13V7l-8-5z" />
    </svg>
  )
}
...
const NAV_ITEMS = [
  { to: '/', end: true, label: 'Dashboard', Icon: IconDashboard, accent: true },
  ...
  { to: '/xp-calculator', label: 'XP Calculator', Icon: IconXP },
...
function App() {
  return (
    <div className="flex min-h-screen flex-col md:flex-row">
      ...
      {/* Desktop sidebar */}
      <aside className="hidden md:fixed md:inset-y-0 md:left-0 md:z-10 md:flex md:w-16 md:flex-col md:items-center md:gap-1 md:border-r-2 md:border-amber-300/15 md:bg-slate-900 md:py-4">
        {NAV_ITEMS.map(({ to, end, label, Icon, accent }) => (
          <NavLink
            key={to}
            to={to}
            end={end}
            ...
            className={({ isActive }) =>
              `group relative flex h-11 w-11 items-center justify-center rounded-xl transition-colors ${
                isActive
                  ...
                  : accent
                  ? 'text-amber-400/50 hover:bg-slate-800 hover:text-amber-400'
      ...
      {/* Page content */}
      <main className="min-w-0 flex-1 md:ml-16">
        <Routes>
          ...
          <Route path="/xp-calculator" element={<XpCalculatorPage />} />
